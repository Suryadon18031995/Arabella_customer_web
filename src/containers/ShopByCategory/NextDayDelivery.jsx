...
import { defaultTimeZone, defaultTimeFormat } from '../../utils/dateUtil';
...
const tomorrow = moment().add(1, 'day').format(defaultTimeFormat);

class NextDayDeliveryContainer extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      unitQty: {},
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!_isEmpty(nextProps.bkmSearchData)) {
      if (JSON.stringify(_get(nextProps.bkmSearchData, 'products.result', {})) !== JSON.stringify(this.state.prevBkmData)) {
        ...
        let farmInfo;
        let temp = {};
        document.title = _get(nextProps.bkmSearchData, 'products.page_title') ? _get(nextProps.bkmSearchData, 'products.page_title') : 'Wholesale Flowers, Wedding Flowers, Bulk Flowers | Bloomkonnect.com';
        ...
        listDataTemp && listDataTemp.length &&
          listDataTemp.map((valTemp) => {
            ...
            if (!_isEmpty(val.delivery)) {
              const filterTmrwsDate = _filter(val.delivery, ['delivery_date', tomorrow]);
              if (!_isEmpty(filterTmrwsDate)) {
                const cutoffTime = _get(filterTmrwsDate[0], 'cutoff_time');
                const a = momentTz().tz(defaultTimeZone);
                //  const b = a.add(momentTz.tz(_get(cutoffTime, 'delivery_date'), 'hh:mm:ss', defaultTimeZone));
                const b = momentTz.tz(cutoffTime, 'HH:mm:ss', defaultTimeZone);
                const timeDiff = b.diff(a, 'seconds') * 1000;
                val.info = { ...val.info, timeDiff };
              } else {
                const filterDate = val.delivery[0];
                if (!_isEmpty(filterDate)) {
                  const cutoffTime = _get(filterDate, 'cutoff_time');
                  const a = momentTz().tz(defaultTimeZone).add(1, 'days');
                  //  const b = a.add(momentTz.tz(_get(cutoffTime, 'delivery_date'), 'hh:mm:ss', defaultTimeZone));
                  //  const b = momentTz.tz(cutoffTime, 'HH:mm:ss', defaultTimeZone);
                  //  const a = momentTz.tz(_get(filterDate[0], 'delivery_date'), defaultTimeFormat, defaultTimeZone);
                  //  const b = a.add(momentTz.tz(_get(cutoffTime, 'delivery_date'), 'hh:mm:ss', defaultTimeZone));
                  // console.log(`${defaultTimeFormat} HH:mm:ss`);
                  const b = momentTz.tz(`${_get(filterDate, 'delivery_date')} ${cutoffTime}`, `${defaultTimeFormat} HH:mm:ss`, defaultTimeZone);
          ...
          <div className='container-block'>
            <div className="col-lg-3 col-md-3 col-sm-6 col-xs-12">
              <ErrorBoundary>
                <FilterComponent
                  ...
                  fromNextDayDelivery={this.state.fromNextDayDelivery}
                />
              </ErrorBoundary>
            </div>
            <div className="col-lg-9 col-md-9 col-sm-9 col-xs-12">
              ...
              <div className="category_description">
                <h4>
                  <p>Want your bulk flowers delivered the very next day? We offer many wholesale flowers that are available for purchase with next day delivery, so you can get fresh products even faster! To get fresh flowers delivered next day, make sure to order by 9am EST Monday through Thursday. Orders placed Thursday at 9am through Monday at 9am will be delivered on Tuesday. Get fresh flowers delivered tomorrow!</p>
                </h4>
              </div> */}
...
const mapStateToProps = (state) => {
  ...
  const {
    bkmCartData, bkmSearchData, filtersData, isFetching: isLoading, error: bkmError,
  } = bkmReducer || [];

  const {
    apiToken, cartId, currencyCode, storeId, loginData, user, storeName, zipcode, error: loginError, cartCount,
  } = loginReducer || [];
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(ErrorHandler(NextDayDeliveryContainer));
