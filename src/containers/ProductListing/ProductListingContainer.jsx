...
import green2blue from '../../assets/svg/66.svg';
...
class ProductListingContainer extends React.Component {
  ...
  UNSAFE_componentWillMount() {
    this.props.getFiltersData({
      currency_code: this.props.currencyCode ? this.props.currencyCode :'USD',
  ...
  componentDidMount() {
    ...
    this.props.getBkmListSearchData({
      store_id: this.props.storeId === '' ? 1 : this.props.storeId,
      currency_code:'USD',
      paage_no: 1,       
      api_token: this.props.apiToken === '' ? '' : this.props.apiToken,
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (!_isEmpty(nextProps.bkmSearchData)) {
      console.log(nextProps.bkmSearchData)
      if (JSON.stringify(_get(nextProps, 'bkmSearchData[0].result', {})) !== JSON.stringify(this.state.prevBkmData)) {
        const detailsTemp = [];
        ...
        let farmInfo;
        let temp = {};
        document.title = _get(nextProps, 'bkmSearchData[0].page_title') ? _get(nextProps, 'bkmSearchData[0].page_title') : 'Mediversal';
        if (this.state.pageNo === 1 || this.state.filtersEnabled) {
          temp = _get(nextProps, 'bkmSearchData[0].result', {});
          listDataTemp = [
            ...Object.keys(temp).map((value, key) => ({ [value]: temp[value] })),
          ];
          count = _get(nextProps, 'bkmSearchData[0].product_count');
          farmInfo = _get(nextProps, 'bkmSearchData[0].farm');
        } else {
          temp = _get(nextProps, 'bkmSearchData[0].result', {});
          listDataTemp = [
            ...this.state.listData,
        ...
        this.setState({
          ...
          farmInfo,
          prevBkmData: temp,
          catDescription: _get(nextProps, 'bkmSearchData[0].cat_description'),
        });
      }
    }
        ...
        if (this.state.productDetails.length < this.state.totalProductCount) {
          if (!this.state.enableClearAll && !this.state.methodUpdated) {
            this.props.getBkmListSearchData({
              ...
              api_token: this.props.apiToken,
              store_id: this.state.selectedStoreId ? this.state.selectedStoreId : this.props.storeId,
  ...
  handleMoreDetailClick = (productId) => {
    this.setState({
      showMoreDetail: { ...this.state.showMoreDetail, [productId]: !_get(this.state.showMoreDetail, productId, false) },
    });
  }
  ...
  render() {
    ...
    return (
      <div>
                <div className="row greenToBlueImage" style={{backgroundImage: `url(${green2blue})`}}>
                  <div className="col-sm-5">
                  </div>
                  <div className="col-sm-7 p-100">	
                  <br/><br/>	
                    <h4 className="f-16w">GET ASSURED PAYPAL CASHBACK VOUCHER BETWEEN</h4>
                    <h2 className="f-45w">RS.40 To 400</h2>
                    <br/>
                    <p className="f-16w">ON YOUR FIRST-EVER PAYPAL TRANSACTION ON MEDIVERSAL</p>
                  </div>
                </div>
                <br/>       
            ...
            <div className='col-lg-9 col-md-9 col-sm-6 col-xs-12'>
              ...
              <ErrorBoundary>
                <InfiniteScroll
                  ...
                  hasMore={(this.state.productDetails.length < this.state.totalProductCount)}
                  loader={this.state.pageNo !== 1 &&
                    <center><span className='infinite-loader-class text-center'>
                      loading more...
                    </span></center>}
                // endMessage={
...
export default connect(mapStateToProps, mapDispatchToProps)(ErrorHandler(ProductListingContainer));
