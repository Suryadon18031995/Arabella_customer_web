...
class AnnualSubscriptionContainer extends React.Component {
  ...
  createChildren = (relatedProducts, details) => relatedProducts && relatedProducts.map((i) => {
    if (details[i]) {
      return (
        <div key={i} className="annual-cat-parent">
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (!_isEmpty(nextProps.seasonalHead)) {
      ...
      const selectSeason = recivedData.filter(each => Number(each.s_id) === 6)[0];
    ...
    if (!_isEmpty(nextProps.addCartResponseDetails) && (this.props.type === 'REQUEST_ADD_TO_CART') && (this.props.cartCount !== _get(nextProps.addCartResponseDetails, ['total_products_in_cart']))) {
      ...
      this.setState(prevState => ({ addedId: prevState.addingId, addingId: '' }));
  ...
  handleChangeInputFields = (event, pid) => {
    const {
      // inputData,
      addTocartButton,
    } = this.state;
    ...
    const selectedDatesArray = { ...this.state.selectedDatesArray, [pid]: delData };
    this.setState({
      addTocartButton,
      selectedDatesArray,
    });
  ...
  handleAddCartClick = (pid) => {
    ...
    if (this.props.apiToken && ((_get(this.props, 'cartType') === 'subscription') || !_get(this.props, 'cartType'))) {
      ...
      addTocartButton[pid] = { ...addTocartButton[pid], isInputQty: '', isSelectedAvailDate: undefined };
  ...
  handleShowPriceBreakDown = prodId => {
    this.setState(prevData => ({
      showPriceBreakDown: { ...prevData.showPriceBreakDown, [prodId]: prevData.showPriceBreakDown && prevData.showPriceBreakDown[prodId] ? !prevData.showPriceBreakDown[prodId] : true }
    }));
  }
  ...
  render() {
    return (
      <div>
        <MetaTags>
          <title>Annual Flower Subscription | Fresh Flowers Subscriptions | BloomKonnect</title>
          <meta name="description" content='Choose our Annual Subscription Program and receive your flowers delivered straight to your door on the same day every week during the subscription period.' />
        </MetaTags>
        <img className='image-responsive custome-desk'
          src='https://d2ob14u1yb5v44.cloudfront.net/media/annual-subscription/subcription-header-banner.jpg'
          alt='Annual Subscription Banner'
          style={{ width: '100%' }} />
        <img className='image-responsive custome-small'
          src='https://d2ob14u1yb5v44.cloudfront.net/media/annual-subscription/Subcription-Page-Header-mobile.png'
          alt='Annual Subscription Banner'
          style={{ width: '100%' }} />
        <BreadCrumbs
        ...
        <div className="container">
          <ErrorBoundary>
            <AnnualSubIndex
              ...
              handleShowPriceBreakDown={this.handleShowPriceBreakDown}
            />
            <ChangeStoreModal
