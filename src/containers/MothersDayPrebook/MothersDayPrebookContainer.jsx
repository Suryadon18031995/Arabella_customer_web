...
class MothersDayPrebookContainer extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      unitQty: {},
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!_isEmpty(nextProps.addCartResponseDetails) && (this.props.type === 'REQUEST_ADD_TO_CART') && (this.props.cartCount !== _get(nextProps.addCartResponseDetails, ['total_products_in_cart']))) {
      ...
      // this.props.setCartType({ cartType: 'pre-book' });
    }
  ...
  handleAddCartClick = (prodData, deliData) => {
    if (this.props.apiToken && ((_get(this.props, 'cartType') === 'pre-book') || (!_get(this.props, 'cartType')))) {
    ...
    } else if (_get(this.props, 'cartType') === 'subscription' || _get(this.props, 'cartType') === 'normal' || _get(this.props, 'cartType') === 'prime') {
  ...
  handleCategorySearch = (event) => {
    ...
    if (!_isEmpty(event.target.value)) {
      const categoryFilterData = this.state.categoryFilterDataTemp.filter(obj => obj.label.toLowerCase().match(searchValue.toLowerCase()));
      this.setState({
        categoryFilterData,
      });
    }
              ...
              <ErrorBoundary>
                ...
                >
                  <div className='shop-flowers-title'>
                    <h1>Mother's Day Pre-Book</h1>
                    <span className='fresh-deal-description'>
                      Reserve the most popular Mother’s Day flowers from BloomKonnect and lock in great pricing.
                      Choose from over 300 of the most popular varieties available: cut-to-order from our partner farms and delivered directly to your door.
                      Our Mother’s Day collection features beautiful Hydrangeas, luscious Roses, Carnations, and a variety of other exquisite stems for life’s leading ladies.
                      Offer ends 11:59 PM EST, 04/14/19.
...
const mapStateToProps = (state) => {
  ...
  const {
    apiToken, currencyCode, storeId, loginData, user, storeName, zipcode, error: loginError, cartCount,
  } = loginReducer || [];

  const error = !_isEmpty(loginError) || _isError(loginError) || !_isEmpty(bkmError) || _isError(bkmError) || !_isEmpty(cartError) || _isError(cartError) || !_isEmpty(wishlistError) || _isError(wishlistError) || !_isEmpty(favouriteError) || _isError(favouriteError);
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(ErrorHandler(MothersDayPrebookContainer));
