...
import LabTestProductDetailComponent from '../../components/ProductComponent/LabTestProductDetailComponent.jsx';
...
import { postAddToCartData, clearCartData, flushCartViewData , postProductAddToCartData } from '../../actions/cart';
...
class LabTestProductDetailContainer extends React.Component {
  constructor(props) {
    super(props);
 ...
 showReviewModal = () => {
  ...
  addProductToCart = () => {
    ...
    const reqBody = ({
      ...
      qty: 1,
      from_time: this.state.selectedStartTime,
      to_time: '',
    });
    console.log(reqBody);
  ...
  createChildren = ({ relatedProducts }) => Object.keys(relatedProducts).map(i =>
      ...
      <div key={i} style={{height: '190px',border: '3px solid #0087b0',width:'250px',position:'relative'}}>
                                <div style={{padding:'25px'}}>
                                  <p>{_get(relatedProducts[i], 'product_name')}</p>
                                  <br/>
                                   <br/>
                                   <b>MRP</b> : {_get(relatedProducts[i], 'regular_rice')}
                                   </div>
  ...
  createUpsellChildrens = ({ upsellProducts }) => Object.keys(upsellProducts).map(i =>
    <center>
      <div key={i} style={{height: '190px',border: '3px solid #0087b0',width:'250px',position:'relative'}}>
                                <div style={{padding:'25px'}}>
                                  <p>{_get(upsellProducts[i], 'name')}</p>
                                  <br/>
                                   <br/>
                                   <b>MRP</b> : {_get(upsellProducts[i], 'price')}
                                   </div>
                                   <div style={{marginBottom: '0px'}}> 
...
export default connect(mapStateToProps, mapDispatchToProps)(ErrorHandler(LabTestProductDetailContainer));
